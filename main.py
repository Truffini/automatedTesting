...
clothing = {
    ...
    'socks': 5.00, 'underwear': 10.00, 'sweatshirt': 40.00, 'checkout': 0
}
...
def get_total(Wic_total, clothing_total, everything_else_total, sales_tax, state):
    if state == 'MA':
        sales_tax_total = everything_else_total * sales_tax
        print("sales tax total - ", sales_tax_total)
        print("your total including tax is - ", (Wic_total + clothing_total + everything_else_total + sales_tax_total))
    else:
...
def main():
    state, sales_tax = get_state()
    Wic_total = get_wic()
    clothing_total = get_clothing()
    everything_else_total = get_everything_else()
    get_total(Wic_total, clothing_total, everything_else_total, sales_tax, state)
